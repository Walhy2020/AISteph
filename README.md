...
- 顶部统一设置面板集中管理增益、快捷键、跟随系统启动、录音目录和在线更新。
...
第一次使用时在客户端窗口中选择实体麦克风，并在右上角“设置”中调整增益。设置面板也可以开启跟随 Windows 启动、查看并打开录音文件夹，以及手动检查更新。此后在任意 Windows 界面按 `Ctrl + Alt + R` 开始录音，再按一次停止并保存。如果最后确认的耳机已经断开，客户端会取消录音并提示，不会自动改用网易虚拟音频或其他麦克风。
