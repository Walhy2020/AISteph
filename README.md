...
- Windows 全局快捷键 `Ctrl + Alt + R`：无需打开网页即可开始或停止录音。
- 服务启动时自动运行托盘快捷键助手，并通过系统通知反馈录音结果。
...
管理台页眉和页脚都会显示当前版本。服务启动后，Windows 托盘会出现 AISteph 快捷键助手。按 `Ctrl+C` 停止服务。

## 全局快捷录音

1. 首次使用时打开网页，选择要使用的实体麦克风并设置录音增益。
2. 此后网页可以关闭；在任意 Windows 界面按 `Ctrl + Alt + R` 开始录音。
3. 再按一次 `Ctrl + Alt + R` 停止录音，音频自动进入资料库。

快捷键严格使用最后确认的实体麦克风。如果华为耳机等默认设备已经断开，会显示系统提示并取消录音，不会自动换成网易虚拟音频或其他麦克风。右键单击系统托盘中的 AISteph 图标，也可以开始、停止或打开工作台。

需要重新编译托盘助手时运行：

```powershell
npm run build:hotkey
```
