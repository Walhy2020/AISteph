...
- Windows 全局快捷键默认是 `Ctrl + Alt + R`，可在设置中重新录入组合键。
- 单实例运行、关闭到托盘、重复启动唤醒已有窗口，并通过系统通知和红色录音图标反馈录音状态。
- 启动时及每 12 小时检查 GitHub Releases 稳定版本，托盘菜单也可手动检查更新。
- 顶部统一设置面板集中管理增益、自定义快捷键、跟随系统启动、录音目录和在线更新。
...
第一次使用时在客户端窗口中选择实体麦克风，并在右上角“设置”中调整增益或重新录入全局快捷键。设置面板也可以开启跟随 Windows 启动、查看并打开录音文件夹，以及手动检查更新。默认在任意 Windows 界面按 `Ctrl + Alt + R` 开始录音，再按一次停止并保存；录音期间任务栏和托盘图标会显示为红底白色 `S`。如果最后确认的耳机已经断开，客户端会取消录音并提示，不会自动改用网易虚拟音频或其他麦克风。
